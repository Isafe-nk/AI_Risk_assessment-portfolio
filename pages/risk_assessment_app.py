            ...
            if q['type'] == 'initial':
                ...
                for sub_q in q['questions']:
                    if sub_q['type'] == 'radio':
                        ...
                        if sub_q['key'] in st.session_state.user_answers:
                            st.write(f"Selected: **{st.session_state.user_answers[sub_q['key']]}**")
                        for i, option in enumerate(sub_q['options']):
                            button_style = "primary" if (sub_q['key'] in st.session_state.user_answers and st.session_state.user_answers[sub_q['key']] == option) else "secondary"
                            if cols[i].button(option, key=f"{sub_q['key']}_{i}", type=button_style):
